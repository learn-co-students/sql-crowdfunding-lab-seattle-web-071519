...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS remaining_amount FROM projects JOIN pledges ON pledges.project_id = projects.id GROUP BY projects.title HAVING remaining_amount >= 0";
###
end
